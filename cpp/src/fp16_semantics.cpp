...
int main() {
    constexpr std::array<std::uint16_t, 24> cases{
    ...
    constexpr std::size_t count = 23;
    std::array<std::uint16_t, 24> lo{}, hi{}, out{};
    lo.fill(0xb800); // -0.5
    hi.fill(0x4000); // +2.0

    const bool available = simd_lab::clamp_f16c(
        out.data(), cases.data(), lo.data(), hi.data(), cases.size());
